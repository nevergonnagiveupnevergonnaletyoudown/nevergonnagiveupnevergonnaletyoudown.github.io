...
const VIZ = {
  ...
  SMOOTH: 0.78,      // 부드럽기(0~1). 높을수록 느긋하게 움직인다
  MIN_DB: -78,       // 이 크기보다 작은 소리는 바닥 취급
  MAX_DB: -12,       // 이 크기면 막대가 꽉 참. 막대가 계속 천장이면 숫자를 올린다

  F_MIN: 45,         // 왼쪽 끝이 담당하는 주파수(Hz)
  F_MAX: 15000,      // 오른쪽 끝이 담당하는 주파수(Hz)
  TILT: 0.55,        // 고음 보정. 고음은 원래 에너지가 작아서 안 올라온다
    ...
    for(let i = 0; i < VIZ.BARS; i++){
      ...
      if(playing && analyser){
        /* 담당 구간에서 제일 큰 값을 쓴다.
           평균을 쓰면 뭉개져서 심심해진다. */
        const [a, b] = ranges[i];
        for(let k = a; k < b; k++) if(bins[k] > v) v = bins[k];
